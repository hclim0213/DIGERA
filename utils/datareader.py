...
class DataReader(object):
    def __init__(self, drug_file, pert_id_file, gene_file, data_file, fp_type, device, fold):
        ...
        if fp_type == 'ecfp':
            ...
            self.drug, self.drug_dim = read_drug_number(drug_file, 4019)
        else:
            raise ValueError("Unknown fingerprint: %s" % fp_type)
        self.gene = read_gene(gene_file, 1107, self.device)

        train_pert_id, test_pert_id = split_data_by_pert_id_cv(pert_id_file, fold)

        # phase 2 : features and labels returns train and test 
        feature_train, feature_test, label_train, label_test, pos_label_train, pos_label_test, \
        neg_label_train, neg_label_test, cell_idx = read_data(data_file, train_pert_id, test_pert_id, self.drug)

        # phase 3 : transfrom_to_tensor returns tensor train and test
        self.train_feature, self.test_feature, self.train_label, self.test_label, \
        self.use_pert_type, self.use_cell_id, self.use_pert_idose, self.pert_type_dim, self.cell_id_dim, self.pert_idose_dim = \
            transfrom_to_tensor(feature_train, feature_test, label_train, label_test, 
                                pos_label_train, pos_label_test, neg_label_train, neg_label_test,
                                self.drug, fp_type, self.device)
