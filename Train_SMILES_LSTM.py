...
if __name__=="__main__":
    ...
    for i, (train_idx, test_idx) in enumerate(skf.split(dataset.x, label)):        
        ...
        model = LSTMModel(input_dim=input_dim, d_model=512, output_dim=output_dim, meta_dim = meta_dim, loss_type=loss_type,
                    d_hid=128, nlayers=1, dropout=drop_out, device=device).to(device)
        ...
        for epoch in tqdm(range(max_epoch)):
            ...
            for batch in train_loader:
                ...
                optimizer.zero_grad()
                output, hidden = model(x, cond, hidden)
            ...
            if best_score < f1score:
                ...
                torch.save({'model_state_dict': model.state_dict(),
                            ...
                            'saved_model/SMILES_LSTM/%s_%s_%d.ckpt' % (model_name + '_' + loss_type,  \
                                                                cell_type, i))
